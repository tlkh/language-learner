...
export function QuizPage() {
  ...
  const tier = tierId ? indexes.quizTiers.get(tierId) : undefined;
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const mastery = await getMasteryMap(pack.code, topic.id, tier.id, variantId);
  ...
  if (!topic || !tier) return <Navigate to={`${base}/topics`} replace />;
  ...
  return (
    <div className="page quiz-page">
      <ScreenHeader
        ...
        description={`Step ${tier.step} of ${pack.quiz.tiers.length}`}
        actions={<Link className="icon-button" to={`${base}/topic/${topic.id}`} aria-label="Leave quiz"><ChevronLeft aria-hidden="true" /></Link>}
      />
      ...
      <main className="quiz-card">
        ...
        <form className="answer-form" onSubmit={submit}>
          ...
          <div className={`answer-input${inputError ? " is-error" : ""}${result ? ` is-${result.status}` : ""}`}>
            <input id="quiz-answer" ref={inputRef} value={input} onChange={(event) => { setInput(event.target.value); if (inputError) setInputError(false); }} lang={question.answerLanguage} autoCapitalize="none" autoComplete="off" autoCorrect="off" spellCheck={false} readOnly={Boolean(result)} aria-invalid={inputError} aria-describedby="answer-helper" placeholder={question.answerPlaceholder} />
