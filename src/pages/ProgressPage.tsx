...
export function ProgressPage() {
  ...
  const selectableVariants = pack.presentation.speechVariantMode !== "primary-with-reference";
  const assessedVariantCount = selectableVariants ? pack.speechVariants.length : 1;
    ...
    <div className="page">
      ...
      {data === undefined ? (
        ...
        <section className="stats-grid" aria-label="Learning statistics">
          ...
          <article><BookCheck aria-hidden="true" /><strong>{stats.passedTiers}</strong><span>steps passed</span><small>of {pack.topics.length * pack.quiz.tiers.length * assessedVariantCount + (phrasePool.length ? 1 : 0)} total</small></article>
      ...
      {data !== undefined && !isFirstSession ? (
        <>
          <section className="breakdown-section" aria-labelledby="breakdown-title">
            <div className="section-heading"><div><h2 id="breakdown-title">Recall breakdown</h2><p>Accuracy by pack-authored quiz step{selectableVariants && pack.speechVariants.length > 1 ? " and speech style" : ""}.</p></div></div>
            <div className="breakdown-grid">
              ...
              {selectableVariants && pack.speechVariants.length > 1 ? variantBreakdown.map(({ variant, correct, attempts: count }) => <article className="breakdown-register" key={variant.id}><strong lang={pack.locale}>{variant.nativeLabel ? `${variant.nativeLabel} · ` : ""}{variant.label}</strong><small>{accuracyLabel(correct, count)}</small></article>) : null}
            </div>
          </section>
          ...
          <section className="activity-section" aria-labelledby="activity-title">
            ...
            <ul className="activity-list">{attempts.slice(0, 6).map((attempt) => {
              ...
              return <li key={attempt.id}><span className={attempt.correct ? "activity-mark is-correct" : "activity-mark"} aria-hidden="true">{attempt.correct ? <Check /> : <X />}</span><span className="sr-only">{attempt.correct ? "Correct" : "Incorrect"}</span><span><strong>{topic?.shortTitle ?? pack.name}</strong><small>{tier?.shortTitle ?? attempt.tierId}{selectableVariants && pack.speechVariants.length > 1 ? ` · ${variant?.label ?? attempt.variantId}` : ""}</small></span><time dateTime={new Date(attempt.answeredAt).toISOString()}>{new Intl.DateTimeFormat(undefined, { month: "short", day: "numeric" }).format(attempt.answeredAt)}</time></li>;
            })}</ul>
          </section>
        </>
      ) : null}
